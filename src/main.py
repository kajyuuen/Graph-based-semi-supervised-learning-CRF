...
if __name__ == "__main__":
    ...
    if(args.crf):
        ...
        if(args.load):
            ...
            # Training CRF
            X_train = [sent2features(s) for s in labeled_sents]
            y_train = [sent2pos(s) for s in labeled_sents]
            crf = sklearn_crfsuite.CRF(
                algorithm='l2sgd',
                c2=0.01,
                max_iterations=100,
            ...
            # Create unlabel features
            X_test = [sent2features(s) for s in unlabeled_sents]
            contextualfeatures_list = [sent2contextualfeature(s) for s in unlabeled_sents]
            ngrams_list = [sent2trigrams(s) for s in unlabeled_sents if len(sent2trigrams(s)) != 0 ]
